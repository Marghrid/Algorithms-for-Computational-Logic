...
if __name__ == "__main__":
	...
	for solver_i in solvers:
		for search in [searches.SAT_UNSAT(3, upper_bound), searches.UNSAT_SAT(3, upper_bound), searches.Binary(3, upper_bound)]:
			solver = solver_dir + solver_i
			if print_time:
			...
			if print_time:
				...
				print(f'({"z3  " if "z3" in solver else "cvc4"}, {search.__class__.__name__.ljust(9)}) -> calls = {num_solver_calls}, time = {solver_time}')
				results[f'({"z3  " if "z3" in solver else "cvc4"}, {search.__class__.__name__.ljust(9)})'] = solver_time

	winner = min(results.items(), key=operator.itemgetter(1))[0]
	if results[winner] > 1:
		print('winner', winner, "time =", results[winner])
